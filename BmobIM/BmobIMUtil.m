...
+(void)pushContentMessageWithTargetUser:(BmobChatUser *)user
    ...
    [query getObjectInBackgroundWithId:user.objectId block:^(BmobObject *object, NSError *error) {
        ...
        if (msg.msgType == MessageTypeText) {
            NSString * string = [NSString stringWithFormat:@"%@",msg.content];
            if(string.length > 20){
            string = [NSString stringWithFormat:@"%@...",[string substringToIndex:20]];
            }
            [dic setObject:@{@"sound":@"",@"badge":@0,@"alert":string} forKey:@"aps"];
        ...
        [dic setObject:user.objectId forKey:PUSH_KEY_TOID];
