...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    detailwish.object = wish.obj;
    detailwish.chatid = wish.charid;
    detailwish.context = wish.context;
    detailwish.nickname = wish.name;
    detailwish.avatar = wish.avatar;
    detailwish.image = wish.image;
    
    NSDateFormatter *date=[[NSDateFormatter alloc] init];
    [date setDateFormat:@"yyyy-MM-dd"];
    NSDate *d=[date dateFromString:wish.age];
    detailwish.timeDate = wish.attime;
    NSTimeInterval dateDiff = [d timeIntervalSinceNow];
    int age=trunc(dateDiff/(60*60*24))/365;
    detailwish.age = [NSString stringWithFormat:@"%d",-age];
//    NSArray * barr = [wish.age componentsSeparatedByString:@"-"];
//    detailwish.sign = [constellation getAstroWithMonth:[barr[1] intValue] day:[barr[2] intValue]];
    detailwish.context = wish.context;
    detailwish.like = [NSString stringWithFormat:@"%d",[wish.like intValue]];
    detailwish.comment= [NSString stringWithFormat:@"%d",[wish.comment intValue]];
    NSString * s = [NSString stringWithFormat:@"%@   %@",wish.distance,[mistiming returnUploadTime:[NSString stringWithFormat:@"%@",wish.attime]]];
    detailwish.distance = s;
    detailwish.attime = [NSString stringWithFormat:@"%@",[mistiming returnUploadTime:[NSString stringWithFormat:@"%@",wish.attime]]];
