...
NSData* EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    ...
    NSString *timeNow = [NSString stringWithFormat:@"%ld", time(NULL)];
    NSTimeInterval timelong = [RecordAudio getAudioTime:out];
    
    NSString *timelongStr = [NSString stringWithFormat:@"%d", (int)round(timelong)];
    NSString *str = [NSString stringWithFormat:@"%@&%@",timeNow, timelongStr];
    [voiceTime saveVoiceTime:str];
    //创建文件夹
    [souceDownload createSaveDir:@"voiceDir"];
    NSString *pathDir = [NSString stringWithFormat:@"%@/Caches/%@", NSHomeDirectory(), @"voiceDir"];
    NSString *wavFile        = [pathDir
                                stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.amr", str]];
